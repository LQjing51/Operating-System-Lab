...
void setup_vm()
{
    clear_pgdir(PGDIR_PA);//0x5e000000lu , use bootblock's page as PGDIR
    ...
    for (uint64_t kva = 0xffffffc050200000lu;
         kva < 0xffffffc060000000lu; kva += 0x200000lu) {// 2M 
        map_page(kva, kva2pa(kva), early_pgdir);//0xffff_ffc0_5020_0000 ~ 0xffff_ffc0_6000_0000 
    }
    // map boot address
    for (uint64_t pa = 0x50000000lu; pa < 0x50400000lu;
         pa += 0x200000lu) {
        map_page(pa, pa, early_pgdir);//5000_0000~5040_0000
    }
    enable_vm();
}
