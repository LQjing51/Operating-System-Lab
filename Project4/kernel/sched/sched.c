...
reg_t do_fork(){
   ...
   // pcb[process_id].tmp_priority = priority;
    ...
    pcb[i].kernel_sp = pcb[process_id].kernel_sp - sizeof(regs_context_t) - sizeof(switchto_context_t);
    regs_context_t *pt_regs =
...
void do_exit(void){
    ...
    if(current_running[cpu_id]->mode == AUTO_CLEANUP_ON_EXIT){
        ...
        if(current_running[cpu_id]->is_thread == 0)
            current_running[cpu_id]->need_clean = 1;
        //ptr_t pt_addr = current_running[cpu_id]->satp;
        //uintptr_t ppn = pt_addr & ((1lu << 44) - 1);
        //freePage(ppn);
...
pid_t do_mthread_create(void (*start_routine)(void*), void *arg){
    ...
    //pid_t pid = current_running[cpu_id]->pid;
