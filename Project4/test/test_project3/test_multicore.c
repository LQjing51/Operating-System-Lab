...
void test_multicore(void)
{
    ...
    struct task_info task_add = {(uintptr_t)&add_task,
                              USER_PROCESS};
    struct TestMultiCoreArg singleCoreArg = {10, 0, MAX_RANGE, &single_core_result};
    ...
    printf("single core: %ld ticks, result = %d            ", singleCoreEnd - singleCoreBegin, single_core_result);

    struct TestMultiCoreArg multiCoreArgs[NUM_CPUS];
    pid_t pids[NUM_CPUS];
    int multi_core_results[NUM_CPUS] = {0};
    for (int i = 0; i < NUM_CPUS; ++i) {
        multiCoreArgs[i].print_location = i + 10;
    ...
    printf("multi core: %ld ticks, result = %d             ", multiCoreEnd - multiCoreBegin, multi_core_final_result);
...
void add_task(struct TestMultiCoreArg* args)
{
    ...
    sys_move_cursor(50, print_location);
    printf("Done ");
    *args->result = result;
    sys_exit();
}
