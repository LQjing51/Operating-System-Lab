...
int sys_ln(char* dir,char* src){
    return invoke_syscall(SYSCALL_LN,dir,src,IGNORE,IGNORE);
}
int sys_ls_l(char* filename){
    return invoke_syscall(SYSCALL_LS_L,filename,IGNORE,IGNORE,IGNORE);
}
int sys_rm(char* filename){
    return invoke_syscall(SYSCALL_RM,filename,IGNORE,IGNORE,IGNORE);
}
int sys_lseek(int fd,int offset,int whence){
    return invoke_syscall(SYSCALL_LSEEK,fd,offset,whence,IGNORE);
}
int sys_touch(char* filename){
    return invoke_syscall(SYSCALL_TOUCH,filename,IGNORE,IGNORE,IGNORE);
}
int sys_cat(char* filename){
    return invoke_syscall(SYSCALL_CAT,filename,IGNORE,IGNORE,IGNORE);
}
int sys_mkfs(){
...
int sys_cd(char* directories){
    return invoke_syscall(SYSCALL_CD,directories,IGNORE,IGNORE,IGNORE);
}
int sys_mkdir(char* directory){
    return invoke_syscall(SYSCALL_MKDIR,directory,IGNORE,IGNORE,IGNORE);
}
int sys_rmdir(char* directory){
    return invoke_syscall(SYSCALL_RMDIR,directory,IGNORE,IGNORE,IGNORE);
}
int sys_ls(char* directories){
    return invoke_syscall(SYSCALL_LS,directories,IGNORE,IGNORE,IGNORE);
}
int sys_fopen(int filename,int mode){
    return invoke_syscall(SYSCALL_FOPEN,filename,mode,IGNORE,IGNORE);
}
int sys_fwrite(int fd, char* context, int length){
    return invoke_syscall(SYSCALL_FWRITE,fd,context,length,IGNORE);
}
int sys_fread(int fd, char *buff, int length){
    return invoke_syscall(SYSCALL_FREAD,fd,buff,length,IGNORE);
}
