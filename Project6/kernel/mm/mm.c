...
void map_net_page(uint64_t va, uint64_t pa, PTE *pgdir)
{
    va &= VA_MASK;
    uint64_t vpn2 =
        va >> (NORMAL_PAGE_SHIFT + PPN_BITS + PPN_BITS);
...
uintptr_t clean_and_rebuild(uintptr_t pgaddr,uintptr_t va){
    ...
    if(pt_addr[vpn2]){
        second_entry = (PTE *)pa2kva(get_pa(pt_addr[vpn2]));
        if(second_entry[vpn1] & _PAGE_PRESENT){
            second_entry[vpn1] = 0;
        }
        pt_addr[vpn2] = 0;
    }
    uint64_t kva = pa2kva(va);
    // map_page_kernel(kva, va, pa2kva(PGDIR_PA));
    // map_page(kva,va,PGDIR_PA);
    local_flush_tlb_all();
    return 0;    
}
void clean_direct_pg(){
    for(uint64_t pa = 0x50000000lu;pa < 0x50400000lu;pa+=200000lu){
        clean_and_rebuild(PGDIR_PA,pa);
    }
}